...
#include "EFM8UB2_USB0_ReportHandler.h"
...
{                                      // status packet to the host
	...
	case OUT_INTERFACE:              // See if recipient was interface
		if ((USB0_STATE != DEV_CONFIGURED) || SETUP.wIndex.c[MSB]
				|| SETUP.wIndex.c[LSB])
	...
	case OUT_ENDPOINT:               // See if recipient was an endpoint
		if ((USB0_STATE != DEV_CONFIGURED) || SETUP.wIndex.c[MSB]) // Make sure device is configured
																   // and index msb = 0x00
			...
			if (SETUP.wIndex.c[LSB] == IN_EP1) {
				if (EP_STATUS[1] == EP_HALT) {         // If endpoint is halted,
													   // return 0x01,0x00
...
{                                      // feature for endpoint 1
	...
	if ((USB0_STATE != DEV_CONFIGURED)
			...
			(SETUP.bmRequestType == IN_DEVICE)
			|| (SETUP.bmRequestType == IN_INTERFACE) || SETUP.wValue.c[MSB]
			|| SETUP.wIndex.c[MSB] || SETUP.wLength.c[MSB]
			|| SETUP.wLength.c[LSB]) {
		Force_Stall();                  // Otherwise send stall to host
	}
...
void Set_Address(void)                // Set new function address
{
	...
	if ((SETUP.bmRequestType != IN_DEVICE)
			...
			SETUP.wIndex.c[MSB] || SETUP.wIndex.c[LSB] || SETUP.wLength.c[MSB]
			|| SETUP.wLength.c[LSB] || SETUP.wValue.c[MSB]
...
// sets the endpoint status to transmit
	...
	if (SETUP.wValue.c[MSB] == DSC_DEVICE || SETUP.wValue.c[MSB] == DSC_CONFIG
			|| SETUP.wValue.c[MSB] == DSC_STRING
			|| SETUP.wValue.c[MSB] == DSC_INTERFACE
			|| SETUP.wValue.c[MSB] == DSC_ENDPOINT) {
		if ((SETUP.wLength.c[LSB] < DATASIZE) && (SETUP.wLength.c[MSB] == 0)) {
...
void Set_Interface(void) {
	...
	if ((SETUP.bmRequestType != IN_INTERFACE)
			...
			|| SETUP.wValue.c[LSB] || SETUP.wIndex.c[MSB]
			|| SETUP.wIndex.c[LSB]) {
